...
- **`/partners`** — Config-driven page (see `/admin/pages/partner`) rendering a banner slideshow with prev/next arrows, a partner directory heading + search/filter toolbar with grid & map views, "Why Partner" benefit cards, a "How It Works" steps strip, and a CTA banner. Partner cards/detail pages are driven by `/api/partners` (admin DB) and hidden on home when empty.
...
- **Partner page content:** the storefront partners page (`/partners`) is
  ...
  `gg_admin_partner_config`, single row `id = 1`): a banner slideshow (up to 5
  uploaded images + title/subtitle, 1920 × 460 px), the partner directory heading
...
- Partner configuration: `gg_admin_partner_config` (single row, `id = 1`) stores
  the storefront partners page JSONB config (banner slideshow images + copy,
  directory heading, benefit card and step lists, CTA buttons, per-section
  visibility/ordering), written by
