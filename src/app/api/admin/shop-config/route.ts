...
import { shopConfigSchema } from "@/lib/schemas";
...
/** GET /api/admin/shop-config — current shop page configuration. */
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const parsed = shopConfigSchema.safeParse(body);
    if (!parsed.success) {
    ...
    await query(
      `INSERT INTO gg_admin_shop_config (id, config, updated_at)
    ...
    console.error("PUT /api/admin/shop-config failed", error);
    return NextResponse.json(
