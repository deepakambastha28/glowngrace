...
import { careerConfigSchema } from "@/lib/schemas";
...
/** GET /api/admin/career-config — current career page configuration. */
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const parsed = careerConfigSchema.safeParse(body);
    if (!parsed.success) {
    ...
    await query(
      `INSERT INTO gg_admin_career_config (id, config, updated_at)
    ...
    console.error("PUT /api/admin/career-config failed", error);
    return NextResponse.json(
